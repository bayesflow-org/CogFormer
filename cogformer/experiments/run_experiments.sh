...
#
# Outputs:
#   Checkpoints : cogformer/experiments/checkpoints/
#   Figures     : cogformer/experiments/figures/
#   Data        : cogformer/experiments/data/
#   Ablations   : cogformer/experiments/ablations/component_ablation_{data,figures}/
#
# Usage:
#   bash cogformer/experiments/run_experiments.sh
...
family_cf_checkpoint() {
    local STEM=$1
    echo "./cogformer/experiments/checkpoints/fm/${2}/${STEM}_l${NUM_LAYERS}_h${NUM_HEADS}_p${PROJ_DIM}_s${NUM_SEEDS}_d${SEED_DIM}_o${NUM_OBS}_b${TRAIN_BATCH}_e${EPOCHS}_t${STEPS_PER_EPOCH}.pt"
}

model_class_checkpoint() {
    echo "./cogformer/experiments/checkpoints/fm/model_class/cogformer_model_class_fm_l${NUM_LAYERS}_h${NUM_HEADS}_p${PROJ_DIM}_s${NUM_SEEDS}_d${SEED_DIM}_b${TRAIN_BATCH}_e${EPOCHS}_t${STEPS_PER_EPOCH}.pt"
}
...
run_family() {
    ...
    for CASE in $CASES; do
        ...
        python cogformer/experiments/model_families/family_bf_train.py \
            --model_family "$FAM" \
            --case         "$CASE" \
            --epochs       "$EPOCHS" \
        ...
        python cogformer/experiments/model_families/family_bf_validate.py \
            --model_family "$FAM" \
            --case         "$CASE" \
    ...
    # CogFormer: train
    echo "--- CF train [${FAM^^}] ---"
    python cogformer/experiments/model_families/family_cf_train.py \
        --model_family      "$FAM" \
        --epochs            "$EPOCHS" \
        --steps_per_epoch   "$STEPS_PER_EPOCH" \
    ...
    python cogformer/experiments/model_families/family_cf_validate.py \
        --model_family  "$FAM" \
        --checkpoint    "$CKPT" \
        --data_dir      "$DATA_DIR" \
        --batch_size    "$VAL_BATCH" \
...
python cogformer/experiments/model_classes/model_class_cf_train.py \
    --epochs            "$EPOCHS" \
    --steps_per_epoch   "$STEPS_PER_EPOCH" \
    --train_batch_size  "$TRAIN_BATCH" \
    --val_batch_size    "$VAL_BATCH" \
...
python cogformer/experiments/model_classes/model_class_cf_validate.py \
    --checkpoint    "$MC_CKPT" \
    --data_dir      "$DATA_DIR" \
    --batch_size    "$VAL_BATCH" \
    --num_samples   "$FM_NUM_SAMPLES" \
...
ABL_DATA="./cogformer/experiments/ablations/component_ablation_data"

run_ablation() {
    local ABLATION=$1
    echo "--- Ablation train: ${ABLATION} ---"
    python cogformer/experiments/ablations/cf_component_ablation_train.py \
        --ablation          "$ABLATION" \
        --epochs            "$EPOCHS" \
        --steps_per_epoch   "$STEPS_PER_EPOCH" \
        --train_batch_size  "$TRAIN_BATCH" \
...
for CONDITION in baseline no_sab no_mab no_film no_fourier; do
    ...
    python cogformer/experiments/ablations/cf_component_ablation_eval.py \
        --condition     "$CONDITION" \
        --checkpoint    "$CKPT" \
        --batch_size    "$VAL_BATCH" \
        --num_samples   "$FM_NUM_SAMPLES" \
...
python cogformer/experiments/ablations/cf_component_ablation_compare.py
