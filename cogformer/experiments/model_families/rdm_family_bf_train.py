...
CASE_CONFIGS = {
    ...
    "fixed": {
        ...
        "flatten_param_outputs": False,
        "squeeze_outputs": True,
        "expected_num_active": 4,  # v, v_diff, a, tau
    },
    "regressed": {
        "free_intrinsics": ["v", "v_diff", "a", "tau", "s_v", "s_tau"],
        "fixed_intrinsics": [],
    ...
    "fixed_regressed": {
        ...
        "flatten_param_outputs": True,
        "squeeze_outputs": False,
        "expected_num_active": 8,  # 4 intercepts (v, v_diff, a, tau) + 2 u_1 + 2 u_2
    },
    "interaction": {
        "free_intrinsics": ["v", "v_diff", "a", "tau", "s_v", "s_tau"],
        "fixed_intrinsics": [],
...
def test(case: str):
    ...
    rdm_samples = rdm_family_simulator.sample(4)

    print(f"Case: {case}")
    for k, v in rdm_samples.items():
    ...
    masks = rdm_samples["masks"]
    params = rdm_samples["params"]
    ...
    adapted_sims = adapter(rdm_family_simulator.sample(4))
    print("\nAdapted samples:")
...
def main(
    ...
    checkpoint_path = f"./cogformer/experiments/checkpoints/rdm_families_bf_{case}"

    workflow = bf.BasicWorkflow(
        simulator=rdm_family_simulator,
        adapter=adapter,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="RDM Model Family Training")
    parser.add_argument(
        "--case",
        type=str,
