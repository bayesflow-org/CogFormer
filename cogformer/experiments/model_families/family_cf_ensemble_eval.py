...
FAMILY_REGISTRY = {
    "ddm": {
        ...
        "outdir_default": "./cogformer/experiments/figures/fm/ddm/ensemble/",
        "fig_stem": "ddm",
        "case_labels": {
    ...
    "rdm": {
        ...
        "fixed_params": ["s_v", "s_tau"],
        "default_fixed_values": {"s_v": 0.0, "s_tau": 0.0},
        "outdir_default": "./cogformer/experiments/figures/fm/rdm/ensemble/",
        "fig_stem": "rdm",
        "case_labels": {
    ...
    "cdm": {
        ...
        "fixed_params": ["s_v", "s_tau"],
        "default_fixed_values": {"s_v": 0.0, "s_tau": 0.0},
        "outdir_default": "./cogformer/experiments/figures/fm/cdm/ensemble/",
        "fig_stem": "cdm",
        "case_labels": {
            "intercept_only": "Intercept Only",
            "regressed":       "Regressed",
...
def main():
    ...
    free_params = reg["free_params"]
    fixed_params = reg["fixed_params"]
    case_labels = reg["case_labels"]
    ...
    model_family = NestedModelFamily(
        model=reg["model_cls"](), name=reg["name"], prior_fun=reg["prior_fun"](),
    ...
    outdir = Path(args.outdir if args.outdir is not None else reg["outdir_default"])
    ...
    for cfg_name, design_config in benchmark.items():
        ...
        samples = model_family.batch_sample(
            **model_family_config,
            ...
            design_config=design_config, link_fun=reg["link_fun"](),
        )
    ...
    for plot_fn, plot_name in [
        ...
        fig_path = outdir / f"{fig_stem}_ensemble_benchmark_{plot_name}.pdf"
    ...
    for rand_dc in random_design_configs:
        ...
        rand_samples = model_family.batch_sample(
            **model_family_config,
            ...
            min_num_regressors=0, fixed_config=True,
            batch_size=args.batch_size, flatten_param_outputs=True,
            design_config=rand_dc, link_fun=reg["link_fun"](),
        )
    ...
    for plot_fn, plot_name in [
        ...
        fig_path = outdir / f"{fig_stem}_ensemble_random_{plot_name}.pdf"
        fig.savefig(fig_path, bbox_inches="tight")
        plt.close(fig)
        logging.info(f"[saved] {fig_path}")
