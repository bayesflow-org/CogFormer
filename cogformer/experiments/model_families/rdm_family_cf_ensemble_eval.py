...
def get_benchmark_design_configs():
    ...
    return {
        ...
        "regressed": {
            "1": intrinsic_params,
            "u_1": ["v_diff", "a"], "u_2": ["v_diff", "a"], "u_1:u_2": [],
        },
        "fixed": {
...
def parse_args():
    ...
    p.add_argument("--outdir", type=str, default="./cogformer/experiments/figures/fm/rdm/ensemble/")
...
def main():
    ...
    for plot_fn, plot_name in [
        ...
        fig_path = outdir / f"rdm_ensemble_benchmark_{plot_name}.pdf"
    ...
    for rand_dc in random_design_configs:
        ...
        rand_samples = model_family.batch_sample(
            **model_family_config,
            ...
            design_config=rand_dc, link_fun=rdm_link_fun(),
        )
    ...
    for plot_fn, plot_name in [
        ...
        fig_path = outdir / f"rdm_ensemble_random_{plot_name}.pdf"
        fig.savefig(fig_path, bbox_inches="tight")
        plt.close(fig)
        logging.info(f"[saved] {fig_path}")
