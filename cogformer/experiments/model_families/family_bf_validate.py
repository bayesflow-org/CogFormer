...
def test(family: str, case: str):
    ...
    checkpoint_path = f"./cogformer/experiments/checkpoints/{reg['checkpoint_prefix']}_{case}/model.keras"
...
def main(family: str, case: str, batch_size: int = 200, num_samples: int = 200, skip_posteriors: bool = False, skip_log_gamma: bool = True):
    ...
    figures_dir = Path(f"./cogformer/experiments/figures/bf/{fam_lower}/{case}")
    ...
    metrics_df.to_csv(figures_dir / f"{fam_lower}_family_{case}_bf_metrics.csv")
