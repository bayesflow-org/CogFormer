...
CASE_CONFIGS = {
    ...
    "fixed": {
        ...
        "max_num_categories": 0,
        "fixed_config": True,
        "design_config": {"1": ["v", "v_diff", "a", "tau"]},
    ...
    "regressed": {
        ...
        "fixed_config": True,
        "design_config": {"1": ["v", "v_diff", "a", "tau", "s_v", "s_tau"], "u_1": ["v_diff", "a"], "u_2": ["v_diff", "a"], "u_1:u_2": []},
        "flatten_param_outputs": True,
        "squeeze_outputs": False,
        "param_names": [
    ...
    "fixed_regressed": {
        ...
        "max_num_categories": 2,
        "fixed_config": True,
        "design_config": {"1": ["v", "v_diff", "a", "tau"], "u_1": ["v_diff", "a"], "u_2": ["v_diff", "a"], "u_1:u_2": []},
        "flatten_param_outputs": True,
        "squeeze_outputs": False,
        "param_names": [
            r"$v$", r"$v_{\mathrm{diff}}$", r"$a$", r"$\tau$",
            r"$u_{1, v_{\mathrm{diff}}}$", r"$u_{1, a}$",
            r"$u_{2, v_{\mathrm{diff}}}$", r"$u_{2, a}$",
        ],
    ...
    "interaction": {
        ...
        "param_names": [
            r"$v$", r"$v_{\mathrm{diff}}$", r"$a$", r"$\tau$", r"$s_v$", r"$s_\tau$",
            r"$u_{1, v}$", r"$u_{1, v_{\mathrm{diff}}}$", r"$u_{1, a}$", r"$u_{1, \tau}$", r"$u_{1, s_v}$",
...
def get_benchmark_design_configs():
    ...
    regressed = {"1": intrinsic_params, "u_1": ["v_diff", "a"], "u_2": ["v_diff", "a"], "u_1:u_2": []}
    fixed = {"1": free_params, "u_1": [], "u_2": [], "u_1:u_2": []}
    fixed_regressed = {"1": free_params, "u_1": ["v_diff", "a"], "u_2": ["v_diff", "a"], "u_1:u_2": []}
    interaction = {
        "1": intrinsic_params,
        "u_1": ["v", "v_diff", "a", "tau", "s_v"],
        "u_2": ["v", "v_diff", "a", "tau"],
...
class RDMModelFamilyBF(bf.simulators.Simulator):
...
def test(case: str):
    ...
    # --- Approximator checks ---
    checkpoint_path = f"./cogformer/experiments/checkpoints/rdm_families_bf_{case}/model.keras"
    approximator = keras.saving.load_model(checkpoint_path)
    print("Loaded model")

    val_sims = rdm_family_simulator.sample(100)
    conditions = {
        "rts": val_sims["rts"],
...
def main(case: str, batch_size: int = 200, num_samples: int = 200, skip_posteriors: bool = False, skip_log_gamma: bool = True):
    ...
    data_dir = Path("./cogformer/experiments/data")
    figures_dir = Path(f"./cogformer/experiments/figures/bf/rdm/{case}")
    evals_dir = Path("./cogformer/experiments/evaluations")
    data_dir.mkdir(parents=True, exist_ok=True)
    figures_dir.mkdir(parents=True, exist_ok=True)
    evals_dir.mkdir(parents=True, exist_ok=True)

    val_sims = rdm_family_simulator.sample(batch_size)
    conditions = {
        "rts": val_sims["rts"],
        "choices": val_sims["choices"],
        "design_matrices": val_sims["design_matrices"],
    ...
    np.savez(
        data_dir / f"rdm_{case}_data.npz",
        rts=rts,
        choices=choices,
        true_set=true_set,
        pred_set=pred_set,
    ...
    metrics.to_csv(evals_dir / f"rdm_families_bf_{case}_evaluations.csv", sep=";")
    logging.info("Metric evaluation is now finished.")

    # --- Adaptive diagnostics ---
    intrinsic_params_all = ["v", "v_diff", "a", "tau", "s_v", "s_tau"]
    variable_names_all = [r"$v$", r"$v_{\mathrm{diff}}$", r"$a$", r"$\tau$", r"$s_v$", r"$s_\tau$"]
    ...
    recovery_fig.savefig(figures_dir / f"rdm_family_{case}_bf_recovery.pdf", bbox_inches="tight")
    ...
    coverage_fig.savefig(figures_dir / f"rdm_family_{case}_bf_coverage.pdf", bbox_inches="tight")
    ...
    ecdf_fig.savefig(figures_dir / f"rdm_family_{case}_bf_ecdf.pdf", bbox_inches="tight")
    plt.close(ecdf_fig)
    ...
    metrics_fig.savefig(figures_dir / f"rdm_family_{case}_bf_metrics.pdf", bbox_inches="tight")
    ...
    metrics_df.to_csv(figures_dir / f"rdm_family_{case}_bf_metrics.csv")
    ...
    if not skip_posteriors:
        for i in range(10):
            ...
            posterior_fig.savefig(
                figures_dir / f"rdm_family_{case}_bf_posterior_{i}.pdf", bbox_inches="tight"
            )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="RDM Model Family Validation")
    parser.add_argument(
        "--case",
        type=str,
