...
#
# Usage:
#   bash cogformer/experiments/model_families/run_rdm_bf_all.sh
#   bash cogformer/experiments/model_families/run_rdm_bf_all.sh --epochs 500 --batch_size 128
#
...
for case in "${CASES[@]}"; do
    ...
    echo "------------------------------------------"
    if ! python -m cogformer.experiments.model_families.rdm_family_bf_train \
            --case "$case" --test; then
    ...
    echo " TRAINING: $case"
    echo "------------------------------------------"
    python -m cogformer.experiments.model_families.rdm_family_bf_train \
    ...
    echo "------------------------------------------"
    python -m cogformer.experiments.model_families.rdm_family_bf_validate \
        --case "$case"
done
