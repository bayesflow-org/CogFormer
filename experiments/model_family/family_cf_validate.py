...
def main():
    ...
    for cfg_name, design_config in benchmark.items():
        ...
        fig.savefig(outdir / f"{fig_stem}_{cfg_name}_mf_recovery.pdf", bbox_inches="tight")
        plt.close(fig)
        ...
        coverage.savefig(outdir / f"{fig_stem}_{cfg_name}_mf_coverage.pdf", bbox_inches="tight")
        plt.close(coverage)
        ...
        ecdf.savefig(outdir / f"{fig_stem}_{cfg_name}_mf_ecdf.pdf", bbox_inches="tight")
        plt.close(ecdf)
        ...
        metrics_fig.savefig(outdir / f"{fig_stem}_{cfg_name}_mf_metrics.pdf", bbox_inches="tight")
        ...
        metrics_csv = paths.metrics_mirror(outdir, make=True) / f"{fig_stem}_{cfg_name}_mf_metrics.csv"
        metrics_df.to_csv(metrics_csv)
        logging.info(f"[saved] {metrics_csv}")
        ...
        if not args.skip_posteriors:
            for i in range(10):
                ...
                posterior.savefig(outdir / f"{fig_stem}_{cfg_name}_mf_posterior_{i}.pdf", bbox_inches="tight")
