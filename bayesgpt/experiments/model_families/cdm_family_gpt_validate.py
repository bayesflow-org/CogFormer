...
def main():
    ...
    # Accumulation for ensemble plots
    ensemble_true_list = []
    ensemble_pred_list = []
    ensemble_masks = []
    ensemble_design_configs = []
    ensemble_labels = []
    ...
    for cfg_name, design_config in benchmark.items():
        ...
        pred_path = pred_dir / f"cdm_families_{cfg_name}_gpt_pred.npz"
        ...
        ensemble_pred_list.append(pred_set)
        ensemble_masks.append(params_mask)
    ...
    if not args.skip_ensemble:
        ...
        for plot_fn, plot_name in [
            ...
            fig_path = ensemble_outdir / f"cdm_ensemble_benchmark_{plot_name}.pdf"
        ...
        for plot_fn, plot_name in [
            ...
            fig_path = ensemble_outdir / f"cdm_ensemble_random_{plot_name}.pdf"
            fig.savefig(fig_path, bbox_inches="tight")
            plt.close(fig)
            logging.info(f"[saved] {fig_path}")
