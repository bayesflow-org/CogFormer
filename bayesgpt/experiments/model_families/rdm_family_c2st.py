...
def get_benchmark_design_configs():
    ...
    return {
        ...
        "interaction": {"1": intrinsic_params, "u_1": ["v", "v_diff", "a", "tau", "s_v"], "u_2": ["v", "v_diff", "a", "tau"], "u_1:u_2": ["v", "v_diff", "a"]},
    }
...
def main():
    ...
    for cfg_name, design_config in benchmark.items():
        ...
        gpt_pred_path = Path(args.gpt_pred_dir) / f"rdm_families_{cfg_name}_gpt_pred.npz"
        ...
        mean_acc, joint = run_comparison(
            ...
            outdir, f"rdm_families_{cfg_name}",
        )
        summary_rows.append({"model": "RDM", "case": cfg_name, "scenario": "Family_vs_BF",
                              "mean_accuracy": mean_acc, "joint_c2st": joint})

        # Class vs Family
        if args.mc_pred_dir is not None:
            mc_pred_path = Path(args.mc_pred_dir) / f"rdm_{cfg_name}_gpt_pred.npz"
            if mc_pred_path.exists():
                ...
                mean_acc_mc, joint_mc = run_comparison(
                    ...
                    variable_names, colors, args.max_num_categories, params_mask,
                    outdir, f"rdm_{cfg_name}_mc",
                )
                summary_rows.append({"model": "RDM", "case": cfg_name, "scenario": "Class_vs_Family",
                                      "mean_accuracy": mean_acc_mc, "joint_c2st": joint_mc})
            else:
                logging.warning(f"No MC pred for '{cfg_name}' at {mc_pred_path}, skipping.")

    summary_df = pd.DataFrame(summary_rows)
    summary_path = Path(args.outdir) / "rdm_c2st_summary.csv"
