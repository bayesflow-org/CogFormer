...
class BayesGPTTrainer:
    def __init__(self, gpt, model_class, adapter, use_wandb=False):
        self.gpt = gpt
        self.model_class = model_class
        self.adapter = adapter
        self.use_wandb = use_wandb

    def train(self, train_config, val_config, checkpoint_path="bayesgpt_model_class_fm.pt", fig_path="fig"):
        global_step = 0
        ...
        checkpoint_dir = Path("./bayesgpt/experiments/checkpoints/fm/model_class/")
    ...
    def train_step(self, config, optimizer, scheduler):
        ...
        if config["gradient_clip_norm"] is not None:
            torch.nn.utils.clip_grad_norm_(self.gpt.parameters(), config["gradient_clip_norm"])
    ...
    def val_step(self, config, global_step, fig_path):
        ...
        figures_dir = Path("./bayesgpt/experiments/figures/fm/model_class/recovery")
...
if __name__ == "__main__":
    ...
    logging.info(
        f"Training ModelClass GPT: {train_config['epochs']} epochs, "
    ...
    if args.use_wandb:
        wandb.init(
            project="bayesgpt-model-class",
            tags=["BayesGPT", "ModelClass", "DDM", "RDM", "CDM"],
            config={**train_config, "bayesgpt": bayesgpt_config},
        )

    checkpoint_path = (
        f"bayesgpt_model_class_fm"
